...
class GoogleScholarDB:
    ...
    def add_author(self, a_id, a_name):
        ...
        existing = cursor.execute(
            f"SELECT *  FROM authors WHERE author_id == '{a_id}';"
        ).fetchone()
        if existing:
            if existing != new_row:
                error_log(
                    f"Existing author is not the same as new_row: {existing} != {new_row}"
                )
        else:
    ...
    def add_publication(self, pub_id: str, title: str, year: int, authors: list):
        ...
        # ===================
        existing = cursor.execute(
            f"SELECT publication_cluster_id, title, year, authors FROM publications "
            f"WHERE publication_cluster_id == '{pub_id}';"
        ).fetchone()
        if existing:
            ...
            if new_row[1].lower() != existing[1].lower():
                ...
                if new_row[1].lower()[:min_len] != existing[1].lower()[:min_len]:
                    error_log(
                        f"Existing publication is not the same as in DB, titles are different."
                    )
                    ...
                    warning_log(
                        f"Existing publication is not the same as in DB, Title will be updated."
                    )
            ...
            if new_row[2] != existing[2]:
                if new_row[2] is None:
                    ...
                    warning_log(
                        f"Existing publication is not the same as in DB, Date will be updated."
                    )
                    warning_log(f"   {existing}")
                    warning_log(f"   {new_row}")
                    cursor.execute(
                        "UPDATE publications SET year = ? "
                        f"WHERE publication_cluster_id == '{pub_id}';",
                        (new_row[2],),
                    )
                else:
            ...
            if new_row[3].lower() != existing[3].lower():
                if len(new_row[3].lower()) <= len(existing[3].lower()):
                    ...
                    warning_log(
                        f"Existing publication is not the same as in DB, authors will be updated."
                    )
                    warning_log(f"   {existing}")
                    warning_log(f"   {new_row}")
                    cursor.execute(
                        "UPDATE publications SET authors = ? "
                        f"WHERE publication_cluster_id == '{pub_id}';",
                        (new_row[3],),
                    )
        else:
    ...
    def dump(self):
        authors = int(
            pd.read_sql_query("SELECT COUNT(*) as count from authors", self.con)
            .iloc[0]
            .iloc[0]
        )
        publications = int(
            pd.read_sql_query("SELECT COUNT(*) from publications", self.con)
            .iloc[0]
            .iloc[0]
        )
        authorship = int(
            pd.read_sql_query("SELECT COUNT(*) from authorship", self.con)
            .iloc[0]
            .iloc[0]
        )
        citations = int(
            pd.read_sql_query("SELECT COUNT(*) from citations", self.con)
            .iloc[0]
            .iloc[0]
        )
    ...
    def get_h_index(self, author_id):
        ...
        request = f"""
            WITH my_table AS (
                SELECT 
                    c.publication_cluster_id, COUNT(cited_by) as citation_count 
                    FROM authorship a 
                    LEFT JOIN citations c ON a.publication_cluster_id = c.publication_cluster_id 
                    WHERE author_id = '{author_id}'
                    GROUP BY c.publication_cluster_id
                ),
                citation_counts AS (
                SELECT 0 AS i
                UNION ALL
                ...
                WHERE (SELECT COUNT(*) FROM my_table WHERE citation_count >= i) >= i
                )
                SELECT MAX(i-1) AS result
                FROM citation_counts;
        """
        ...
        return _h_index if _h_index else None
    ...
    def get_citation_graph(self, focus_authors=None):
        ...
        # Get publication ids of interest
        publication_ids = set()
        if focus_authors:
            for author_id in focus_authors:
                cluster_ids = self.get_author_details(author_id)["cluster_id"]
                for cluster_id in cluster_ids:
                    publication_ids.add(cluster_id)
        ...
        # Add nodes for papers with size and color attributes
        for row in citations.itertuples():
            cluster_id = row.cluster_id
            cite_cluster = row.cite_clusters

            G.add_node(cluster_id, size=10, color="blue")
            G.add_node(cite_cluster, size=10, color="blue")

        # Add edges between papers with width attribute
        for row in citations.itertuples():
            cluster_id = row.cluster_id
            cite_cluster = row.cite_clusters
            G.add_edge(cluster_id, cite_cluster, width=2)

        # Highlight papers from specific authors
        for publication_id in publication_ids:
            if publication_id in G.nodes:
                G.nodes[publication_id]["color"] = "red"
                G.nodes[publication_id]["size"] = 20

...
class GoogleScholarCacheSQLite (GoogleScholarCache):
    ...
    def copy_into(self, cache):
        ...
        try:
            authors = (pd.read_sql_query("SELECT * from author_page", self.con)).to_dict(orient='records')
            for author in authors:
                cache.add_author_page(author["author_id"], author["text"])
            publications = (pd.read_sql_query("SELECT * from publication_page", self.con)).to_dict(orient='records')
            ...
            citations = (pd.read_sql_query("SELECT * from citations_page", self.con)).to_dict(orient='records')
            ...
            versions = (pd.read_sql_query("SELECT * from versions_page", self.con)).to_dict(orient='records')
